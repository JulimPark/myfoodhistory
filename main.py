...
st.write('')
st.write('')
df_res = pd.DataFrame({'섭취일':[],'섭취시간':[],'음식명':[],'1인분칼로리(kcal)':[],'탄수화물(g)':[],'단백질(g)':[],'지방(g)':[],'콜레스트롤(g)':[],'식이섬유(g)':[],'나트륨(g)':[]})
for i in foot_names:
    ddf = df[df['음식명']==i].loc[:,['음식명','1인분칼로리(kcal)','탄수화물(g)','단백질(g)','지방(g)','콜레스트롤(g)','식이섬유(g)','나트륨(g)']]
